...
Ошибка в логах: `dial tcp [::1]:5432: connection refused` — приложение подключается к localhost вместо базы Railway. Ничего **вручную вставлять не нужно**. Нужно один раз связать сервис приложения с сервисом PostgreSQL через **Reference**.
...
7. В выпадающем списке выбери **сервис PostgreSQL** (именно базу, не приложение).
8. Дальше выбери **переменную**: **`DATABASE_URL`** или **`POSTGRES_PRIVATE_URL`** (внутренний URL).
9. Нажми **Add** / **Save**.
