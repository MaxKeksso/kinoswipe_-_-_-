...
export const MovieDetails: React.FC<MovieDetailsProps> = ({ movie, isAdmin = false, onClose }) => {
  const [activeTab, setActiveTab] = useState<Tab>(movie.comic_poster_url ? 'comic' : 'info');
  ...
  return (
    <div className="movie-details-overlay" onClick={onClose}>
      <div className="movie-details-content" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Табы — только если есть комикс-постер */}
        {movie.comic_poster_url && (
          <div className="movie-details-tabs">
            <button
              className={`movie-tab${activeTab === 'info' ? ' movie-tab--active' : ''}`}
              onClick={() => setActiveTab('info')}
            >
              Инфо
            </button>
            <button
              className={`movie-tab${activeTab === 'comic' ? ' movie-tab--active' : ''}`}
        ...
        {/* Комикс-постер */}
        {activeTab === 'comic' && movie.comic_poster_url && (
          <div className="comic-poster-section">
            <img
              src={movie.comic_poster_url}
              alt={`Комикс-постер: ${movie.title}`}
              className="comic-poster-img"
            />
          </div>
        )}

        {/* Основной контент (Инфо) */}
        {activeTab === 'info' && (
          <>
